...
:$updated  :$by 
    ...
    set "totaltrq_sql_filial=%totaltrq_sql_dir%\%filial_code%.TRQ"
    
    if not exist %totaltrq_sql_filial% ( exit /b 0 )
    ...
    call "%file_functions%" SAVE_LOG "%app_sql_display%" "%log_success%" "Copiando '%totaltrq_sql_filial%' para '%totaltrq_sql_file%'" 
    copy %totaltrq_sql_filial% %totaltrq_sql_file%
