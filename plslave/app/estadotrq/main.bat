...
:$updated  :$by 
    ...
    call "%file_functions%" SAVE_LOG "%app_display%" "%log_success%" "Verificando processos em '%estadotrq_svn%'..."
    
    
    :: Lista todos os processos para executar
    setlocal ENABLEDELAYEDEXPANSION
    for %%A in (%estadotrq_svn_dir%\*.rar) do (
        if !exec_name!null==null (
            :: Verifica se existe arquivo '.ini'
            if exist "%estadotrq_svn_dir%\%%~nA.ini" (
...
:$updated  :$by
    ...
    set "estadotrq_svn_dir=%svn_dir%%estadotrq_svn%\%app_name%"
    
    :: PLSQLBIGSK
    set "totaltrq_plsqlbigsk=PLSQLBIGSK"
    set "totaltrq_plsqlbigsk_exec=%totaltrq_plsqlbigsk%.EXE"
    set "file_plsqlbigsk=%estadotrq_dir%\%totaltrq_plsqlbigsk_exec%"
    set "totaltrq_plsqlbigsk_origi_log=%estadotrq_dir%\%totaltrq_plsqlbigsk%%filial_code%.LOG
...
:$updated  :$by 
   ...
   for /f "tokens=1,2* delims=^=" %%A in (%estadotrq_svn_dir%\%exec_name%.ini) do (call "%file_functions%" SET_VAR "%%A" "%%B")
...
:$updated  :$by 
    ...
    set "estadotrq_descompacta_dir=%estadotrq_atualize_dir%\%app_name%"
    set "estadotrq_descompacta_rar=%estadotrq_svn_dir%\%exec_name%.rar"

    
    :: Verifica se pasta de backup existe
    if exist %estadotrq_descompacta_dir%\nul (
        :: Apaga a pasta
        call "%file_functions%" SAVE_LOG "%app_descompacta_display%" "%log_success%" "Apagando diretorio '%estadotrq_descompacta_dir%'" 
        call "%file_functions%" DELETE_DIR %estadotrq_descompacta_dir% "/S /Q"
    )
    
    
    :: Descompactando arquivos
    call "%file_functions%" SAVE_LOG "%app_descompacta_display%" "%log_success%" "Descompactando '%estadotrq_descompacta_rar%' em '%estadotrq_atualize_dir%'" 
    %file_unrar% x %estadotrq_descompacta_rar% %estadotrq_atualize_dir%
...
:$updated  :$by 
    ...
    set "estadotrq_sql_dir=%estadotrq_atualize_dir%\%app_name%"
    set "estadotrq_sql_file=%estadotrq_sql_dir%\%estadotrq_plsql_file%"
    
    set "estadotrq_sql_uf=%estadotrq_sql_dir%\%filial_uf%.TRQ"
    
    
    if not exist %estadotrq_sql_uf% (
        call "%file_functions%" SAVE_LOG "%app_display%" "%log_error%" "Arquivo '%estadotrq_sql_uf%' nao foi encontrado"
        exit /b 0
    )
    ...
    :: Copia ESTADO.TRQ para TOTAL.TRQ
    call "%file_functions%" SAVE_LOG "%app_sql_display%" "%log_success%" "Copiando '%estadotrq_sql_uf%' para '%estadotrq_sql_file%'" 
    copy %estadotrq_sql_uf% %estadotrq_sql_file%
    
    
    :: Move TOTAL.TRQ para pasta da aplicação
    call "%file_functions%" SAVE_LOG "%app_sql_display%" "%log_success%" "Copiando '%estadotrq_sql_file%' para '%estadotrq_dir%'" 
    call "%file_functions%" COPY_FILE "%estadotrq_sql_file%" "%estadotrq_dir%\" & if ERRORLEVEL 1 (
