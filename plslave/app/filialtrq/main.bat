...
:$updated  :$by 
    ...
    call "%file_functions%" SAVE_LOG "%app_display%" "%log_success%" "Verificando processos em '%filialtrq_svn%'..."
    
    
    :: Lista todos os processos para executar
    setlocal ENABLEDELAYEDEXPANSION
    for %%A in (%filialtrq_svn_dir%\*.rar) do (
        if !exec_name!null==null (
            :: Verifica se existe arquivo '.ini'
            if exist "%filialtrq_svn_dir%\%%~nA.ini" (
...
:$updated  :$by
    ...
    set "filialtrq_svn_dir=%svn_dir%%filialtrq_svn%\%app_name%"
    
    :: PLSQLBIGSK
    set "totaltrq_plsqlbigsk=PLSQLBIGSK"
    set "totaltrq_plsqlbigsk_exec=%totaltrq_plsqlbigsk%.EXE"
    set "file_plsqlbigsk=%filialtrq_dir%\%totaltrq_plsqlbigsk_exec%"
    set "totaltrq_plsqlbigsk_origi_log=%filialtrq_dir%\%totaltrq_plsqlbigsk%%filial_code%.LOG
...
:$updated  :$by 
   ...
   for /f "tokens=1,2* delims=^=" %%A in (%filialtrq_svn_dir%\%exec_name%.ini) do (call "%file_functions%" SET_VAR "%%A" "%%B")
...
:$updated  :$by 
    ...
    set "filialtrq_descompacta_dir=%filialtrq_atualize_dir%\%app_name%"
    set "filialtrq_descompacta_rar=%filialtrq_svn_dir%\%exec_name%.rar"

    
    :: Verifica se pasta de backup existe
    if exist %filialtrq_descompacta_dir%\nul (
        :: Apaga a pasta
        call "%file_functions%" SAVE_LOG "%app_descompacta_display%" "%log_success%" "Apagando diretorio '%filialtrq_descompacta_dir%'" 
        call "%file_functions%" DELETE_DIR %filialtrq_descompacta_dir% "/S /Q"
    )
    
    
    :: Descompactando arquivos
    call "%file_functions%" SAVE_LOG "%app_descompacta_display%" "%log_success%" "Descompactando '%filialtrq_descompacta_rar%' em '%filialtrq_atualize_dir%'" 
    %file_unrar% x %filialtrq_descompacta_rar% %filialtrq_atualize_dir%
...
:$updated  :$by 
    ...
    set "filialtrq_sql_dir=%filialtrq_atualize_dir%\%app_name%"
    set "filialtrq_sql_file=%filialtrq_sql_dir%\%filialtrq_plsql_file%"
    
    set "filialtrq_sql_filial=%filialtrq_sql_dir%\%filial_code%.TRQ"
    
    if not exist %filialtrq_sql_filial% (
        call "%file_functions%" SAVE_LOG "%app_display%" "%log_error%" "Arquivo '%filialtrq_sql_filial%' nao foi encontrado"
        exit /b 0
    )
    ...
    :: Copia FILIAL.TRQ para TOTAL.TRQ
    call "%file_functions%" SAVE_LOG "%app_sql_display%" "%log_success%" "Copiando '%filialtrq_sql_filial%' para '%filialtrq_sql_file%'" 
    copy %filialtrq_sql_filial% %filialtrq_sql_file%
    
    
    :: Move TOTAL.TRQ para pasta da aplicação
    call "%file_functions%" SAVE_LOG "%app_sql_display%" "%log_success%" "Copiando '%filialtrq_sql_file%' para '%filialtrq_dir%'" 
    call "%file_functions%" COPY_FILE "%filialtrq_sql_file%" "%filialtrq_dir%\" & if ERRORLEVEL 1 (
